...
import dashboardFakerFactory from "./dashboardFakerFactory";
...
const DashboardFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dashboardFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("dashboard").create(elem));
...
export default connect(mapState, mapDispatch)(DashboardFakerDialogComponent);
